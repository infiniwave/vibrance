...
pub async fn fetch_album_art(artist: &str, track: &str) -> Result<String> {
    let mut url =
        Url::parse("https://musicbrainz.org/ws/2/recording/").expect("Should have parsed");
    url.query_pairs_mut()
        .append_pair(
            "query",
            &format!("artist:\"{}\" recording:\"{}\"", artist, track),
        )
...
pub async fn fetch_release_image(release: &str) -> Result<Option<String>> {
    ...
    let images = json
        ...
        .ok_or(anyhow::anyhow!(
            "Invalid response format from Cover Art Archive"
        ))?;
    ...
    if let Some(image) = images.first() {
        if let Some(image_url) = image.get("image") {
            return Ok(Some(
                image_url
                    .as_str()
                    .ok_or(anyhow::anyhow!("Expected 'image' to be a string"))?
                    .to_string(),
            ));
        }
    }
