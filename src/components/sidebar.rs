use gpui::{AppContext, ClickEvent, Entity, IntoElement, ParentElement, Render, Styled, Window, div, rgba};
use gpui_component::{Side, sidebar::{Sidebar as GpuiSidebar, SidebarMenu, SidebarMenuItem, SidebarToggleButton}};
...
impl Sidebar {
    ...
    pub fn item_home(&mut self, _event: &ClickEvent, _window: &mut Window, cx: &mut gpui::Context<'_, Self>) {
        self.navigation_state.update(cx, |state, _| {
            *state = NavigationState::Home;
        });
    }

    pub fn item_search(&mut self, _event: &ClickEvent, _window: &mut Window, cx: &mut gpui::Context<'_, Self>) {
        self.navigation_state.update(cx, |state, _| {
...
impl Render for Sidebar {
    fn render(&mut self, window: &mut gpui::Window, cx: &mut gpui::Context<'_, Self>) -> impl IntoElement {
        ...
        GpuiSidebar::new(Side::Left)
            ...
            .header(div().child(SidebarToggleButton::left().collapsed(*self.collapsed.read(cx))
            ...
            .child(
                SidebarMenu::new().child(
                    ...
                    .active(matches!(self.navigation_state.read(cx), NavigationState::Home))

                ).child(SidebarMenuItem::new("Search")
                    .icon(Icon::Search)
                .on_click(cx.listener(Self::item_search))
                    .active(matches!(self.navigation_state.read(cx), NavigationState::Search)))
            )
