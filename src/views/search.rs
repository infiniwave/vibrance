...
use gpui_component::{Icon as GpuiIcon, Sizable, StyledExt, input::{Input, InputEvent, InputState}};
use tokio::task;

use crate::{components::{icon::Icon, player::Player as PlayerComponent, track_list::{TrackList, TrackListDelegate}}, player::{PLAYER, PlayerCommand, Track}, providers};
...
impl SearchView {
    pub fn new(window: &mut gpui::Window, cx: &mut gpui::Context<Self>, _player_component: Entity<PlayerComponent>) -> Self {
        ...
        let on_play_callback: Arc<dyn Fn(Track) + Send + Sync> = Arc::new(move |track: Track| {
            ...
            task::spawn(async move {
                let track = if track_clone.yt_id.is_some() && track_clone.path.is_none() {
                    ...
                    match providers::youtube::download_track_default(track_clone.yt_id.as_ref().unwrap()).await {
        ...
        let _s = vec![cx.subscribe_in(&input_state, window, move |view,state,event,window,cx| {
            let y = x.clone();
                    ...
                    cx.spawn(async move |e, app| {
                        ...
                        match search_result {
                            Ok(Ok(tracks)) => {
                                ...
                                let items = tracks.iter().map(|t| Track {
                                ...
                                let new_delegate = TrackListDelegate::new(items).with_on_play(on_play);
