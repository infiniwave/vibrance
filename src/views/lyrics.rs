                                ...
                                if let Some(this_entity) = this.upgrade() {
                                    let _ = cx.update_entity(&this_entity, |view, cx| {
                                        match lyrics_result {
                                            Ok(Ok(lyrics_list)) => {
                                                view.loading = false;
                                                if let Some(first_lyrics) = lyrics_list.first() {
                                            ...
                                            Ok(Err(e)) => {
                                                view.loading = false;
                                                view.error =
                                                    Some(format!("Failed to fetch lyrics: {}", e));
                                            }
                                            Err(e) => {
                                                view.loading = false;
                                                view.error = Some(format!("Task error: {}", e));
                                            }
                                        }
                                        cx.notify();
                                    });
    ...
    fn render(
        ...
        gpui::div()
            ...
            .when(
                ...
                |div| div.child(gpui::div().text_sm().child("No lyrics available")),
            )
                    ...
                    gpui::div()
                        ...
                        .children(lyrics.iter().enumerate().map(|(i, line)| {
                            gpui::div()
                                ...
                                .when(i != self.active_line, |d| d.text_color(gpui::rgb(0x888888)))
